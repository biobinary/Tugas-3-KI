...
elements.users?.addEventListener('change', () => {
  ...
  if (selectedUser && userPublicKeys[selectedUser]) {
    elements.recipientKeyDisplay.innerHTML = 
      `<strong>Recipient's Public Key:</strong> <code>${userPublicKeys[selectedUser]}</code>`;
...
socket.on('register_response', (data) => {
  ...
  if (data.ok) {
    ...
    elements.keysSection.classList.remove('hidden');
    ...
    elements.publicKeyDisplay.insertBefore(
      pubKeyText, 
      elements.publicKeyDisplay.firstChild
    );
    
    elements.privateKeyDisplay.insertBefore(
      privKeyText, 
      elements.privateKeyDisplay.firstChild
    );
...
socket.on('receive_message', (data) => {
  ...
  try {
    ...
    msgDiv.innerHTML = `
      <div class="decrypt-badge">✅ Decrypted</div>
    ...
    msgDiv.innerHTML = `
      <div class="message-header">${from}</div>
      <div class="message-content" style="color: #e53e3e;">
